...
with open(csv_file, 'w') as f_csv :
    for json_data in json_load :
        ...
        frame_time  = frame_data["frame.time"]
        pos = frame_time.rfind(' ')
        frame_time  = frame_time[:pos]
        pos = frame_time.rfind(' ')
        frame_time  = frame_time[:pos]
        # frame_time  = frame_time.rstrip(' 東京 (標準時)')     # "東京 (標準時)"部分を削除
        frame_len   = frame_data["frame.len"]
        # print(f'{frame_index},"\'{frame_time}",{frame_len},', end='')
        f_csv.write(f'{frame_index},"\'{frame_time}",{frame_len},')
...
76行目～のforループで各JSONレコードを読み込みながら処理。  

81～84行目で時刻表示の日本語部分を削除。(本当は85行目みたいに書きたかったけどうまく動かなかった。ここもチカラワザ)  
削除しなくても良い場合(表示形式によってはそもそも要らないかも)はコメントアウト(Excelで文字化けしてたので削除したかっただけ)。  

91行目～のforループがデータを取り出す部分。  
