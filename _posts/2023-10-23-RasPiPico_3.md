...
cp ide/vscode/settings.json .vscode/settings.j
```

このままだとビルドが全ビルドになってしまい、時間がかかってしまうのでビルドターゲットを指定できるように変更しておきます。  

具体的には``settings.json`` の ``build`` / ``buildTarget`` / ``launchTarget`` を``"default"``に設定。  
変更後の``settings.json`` は以下のようになります。  
...
```json
{
    ...
    "cmake.statusbar.advanced": {
        ...
        "build": {
            "visibility": "default"     // hiddenから変更(ビルドのみ実行ができる)
        },
        "buildTarget": {
            "visibility": "default"     // hiddenから変更(ビルドターゲットが選択できる)
        },
        "launchTarget":{                // 追加(しなくても大丈夫だけど)
            "visibility": "default"
        }
    },
