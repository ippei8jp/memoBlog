...
> [!WARNING]
> RaspberryPi4ではこの設定ではGUIの解像度は変更できないらしい。(CUIのコンソールはこれで変更できる)   
> GUIの解像度を変更するには、GUI起動後メニューから 「設定」→「Screen Configuration」  
> (またはコンソールから``sudo arandr``)でScreen Layout Editorを起動し  
> - Configure→Screens→HDMI-1→解像度→解像度選択(1920x1080など)  
> - Configure→適用   
> 
> として設定する  
